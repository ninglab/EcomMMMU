    ...
    def __getitem__(self, i) -> Dict[str, List]:      
        ...
        if "image" in source:
            ...
            for image_path in image_sources:
                ...
                try:
                    temp_img = Image.open(requests.get(image_path, stream=True).raw).convert("RGB")
                except:
                    temp_img = Image.open(requests.get("https://m.media-amazon.com/images/I/51Avbz7ty+L._AC_SX679_.jpg", stream=True).raw).convert("RGB")
                images.append(
                    temp_img
                    if self.load_image and temp_img else image_path
                )
